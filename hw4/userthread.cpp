...
static sigset_t signal_set;/* implement a user-space-level-thread library
   Collaborater: Zhanpeng Wang, Elieen Feng
   cite: https://github.com/xxlelementlxx/Thread-Library/blob/master/thread.cc
         https://github.com/ManojkumarMuralidharan/User-level-Thread-library
 */
...
static int scheduler_valgrind_register;
...
int thread_libinit(int policy) {
  ...
  try{
    ...
    main-> valgrind_register = VALGRIND_STACK_REGISTER(main->stack, main->stack + STACKSIZE);
    ...
    scheduler_valgrind_register = VALGRIND_STACK_REGISTER(sch_stack, sch_stack + STACKSIZE);
  ...
  logfile <<"Note: the main thread is thread with TID 0 and unit is microsecond." << endl;
  logfile << "[ticks] \t OPERATION \t TID \t PRIORITY" <<endl;
  return SUCCEED;
}
...
int thread_libterminate(void){
  if(!init)
    return FAILED;
  if(current->tid != 0){
    printf("Warning: should not use lib_terminate in thread other than main!\n");
    return FAILED;
  }
    
  //depends on the policy
  //clean current if not empty
  /*
  if(current->uc != NULL && current -> stack != NULL && current!= NULL){
  ...
  */
  ...
  //clean scheduler
  VALGRIND_STACK_DEREGISTER(scheduler_valgrind_register);
  delete[] sch_stack;
  delete scheduler;
  //clean main
  if(main_thread->uc != NULL && main_thread-> stack != NULL && main_thread != NULL){
    VALGRIND_STACK_DEREGISTER(main_thread->valgrind_register);
  ...
  */
  ...
  logfile << "[" << cur_time - lib_begin << "] \t LIB_TERM  \t 0 \t -1" <<endl;
  logfile.close();
  return SUCCEED;
}
...
int thread_create(void (*func)(void *), void *arg, int priority) {
  ...
  if(!init)
    return FAILED;
  if(type == PRIORITY &&priority != -1 && priority != 0 && priority != 1){
    return FAILED;                                                                                                                           
  }  
  mythread* tmp;
  try {
    ...
    tmp-> valgrind_register = VALGRIND_STACK_REGISTER(tmp->stack, tmp->stack + STACKSIZE);
	...
	tmp->expected_time = total_runtime/total_thread;
      ...
      /*
      //if priority is not ok, we will fail the creation
  ...
  gettimeofday(&tv,NULL);
  cur_time = tv.tv_sec * MILLION + tv.tv_usec;
  logfile << "[" << cur_time-lib_begin << "] \t CREATED \t " << tmp->tid << " \t " <<tmp-> priority << endl;
  return tmp->tid;
}
...
int thread_yield(void) {
  ...
  gettimeofday(&tv, NULL);
  cur_time = tv.tv_sec * MILLION + tv.tv_usec;
  logfile << "[" << cur_time-lib_begin << "] \t STOPPED \t " << current->tid << " \t " << current-> priority << endl;
  return SUCCEED;
}
...
void delete_thread(mythread* cur) {
  gettimeofday(&tv, NULL);
  cur_time = tv.tv_sec * MILLION + tv.tv_usec;
  logfile << "[" << cur_time-lib_begin << "] \t FINISHED \t " << cur->tid << " \t " <<cur-> priority << endl;
  VALGRIND_STACK_DEREGISTER(cur->valgrind_register);
  delete[] cur->stack;
...
static void dispatcher() {
  ...
  if(setitimer(ITIMER_REAL, &clocktick, NULL) < 0)
    exit(1);
    //cout << "Set timer failed!\n" << endl;
  
  mythread* tmp;
  //find the appropriate thread to run
  if(type == FIFO) {
    //move it after the FINISHED state
    /*
    if(fifoq.size() == 0) {
    ...
    */
    ...
    gettimeofday(&tv, NULL);
    cur_time = tv.tv_sec * MILLION + tv.tv_usec;
    logfile << "[" << cur_time-lib_begin << "] \t SCHEDULED \t " << current->tid << " \t " <<current-> priority << endl;
    //swap
    makecontext(scheduler, dispatcher, 0);
    ...
    */
	...
	for(int i = 0; i < current->wait_size ; i++) {
	  for(int j = 0; j < suspended.size() ; j++) {
	    if(current->wait_tids[i] == suspended[j]->tid) {
	      //cout << "we will add " << suspended[j]->tid << endl;
    ...
    */
    ...
    if(current->state == FINISHED){
      ...
      gettimeofday(&tv, NULL);
      cur_time = tv.tv_sec * MILLION + tv.tv_usec;
      logfile << "[" << cur_time-lib_begin << "] \t FINISHED \t " << current->tid << " \t " <<current-> priority << endl;
      */      
      if(!current->wait_tids.empty()) {
        for(int i = 0; i < current->wait_size ; i++) {
          for(int j = 0; j < suspended.size() ; j++) {
    ...
    gettimeofday(&tv, NULL);
    cur_time = tv.tv_sec * MILLION + tv.tv_usec;
    logfile << "[" << cur_time-lib_begin << "] \t SCHEDULED \t " << tmp->tid << " \t " <<current-> priority << endl;
    makecontext(scheduler, dispatcher, 0);
    swapcontext(scheduler, current->uc);
  }
}
