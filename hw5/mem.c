...
int Mem_Init(long sizeOfRegion) {
  if(main_head != NULL) {
    m_error = E_BAD_ARGS;
    return FAILED;#include <stdio.h>#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <sys/mman.h>
#include <unistd.h>
...
int Mem_Init(long sizeOfRegion) {
  ...
  header* first_head = (header*) ((char*)main_head + MIN_BLOCK_SIZE);
...
void *Mem_Alloc(long size) {
  ...
  if(main_head -> remaining_space < size) {
    printf("can't find enough space for request\n");
    m_error = E_NO_SPACE;
    return NULL;
  }
  ...
  main_head->remaining_space -= size;
...
int Mem_Free(void *ptr, int coalesce) {
  ...
  if(validate_ptr(ptr) == SUCCEED) {
    ...
    //update the remaining_space to allow more space to use
    long size = get_block_size(cur);
    //printf("we will free %ld of block\n",size);
    main_head->remaining_space += size;
  }
  else {
...
void Mem_Dump() {
  ...
  printf("####### the user is able to access %ld  bytes of memory now ########\n",main_head->remaining_space);
